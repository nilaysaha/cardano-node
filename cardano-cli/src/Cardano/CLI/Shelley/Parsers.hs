...
pAddressCmd =
  Opt.subparser $
    mconcat
      [ Opt.command "key-gen"
          (Opt.info pAddressKeyGen $ Opt.progDesc "Create a  address key pair.")
...
pMultiHostName =
 ...
 where
  ...
  pDNSName = Opt.option (Opt.eitherReader eDNSName)
               (  Opt.long "multi-host-name-pool-relay-dns"
               <> Opt.metavar "STRING"
               <> Opt.help "The stake pool relay's DNS name (An SRV DNS record)"
               )
...
pSingleHostName =
 ...
 where
  pDNSName :: Parser Shelley.DnsName
  pDNSName = Opt.option (Opt.eitherReader eDNSName)
               (  Opt.long "host-name-pool-relay-dns"
               <> Opt.metavar "STRING"
               <> Opt.help "The stake pool relay's DNS name (An A or AAAA DNS record)"
               )
